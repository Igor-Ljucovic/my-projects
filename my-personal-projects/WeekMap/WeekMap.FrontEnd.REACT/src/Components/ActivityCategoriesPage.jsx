  ...
  return (
    <div style={containerStyle}>
      ...
      <form onSubmit={handleSubmit} style={{ marginBottom: '10px' }}>
        ...
        <button type="submit" style={primaryButtonStyle}>{isEditing ? 'Save Changes' : 'Add Category'}</button>
        {isEditing && (
        ...
        <ul style={{ listStyle: 'none', padding: 0 }}>
          {categories.map(cat => (
            <li key={cat.activityCategoryID} style={{
              ...
              <div>
                <button onClick={() => handleEditClick(cat)} style={secondaryButtonStyle}>Edit</button>
                <button onClick={() => handleDeleteCategory(cat.activityCategoryID)} style={dangerButtonStyle}>Delete</button>
              </div>
            </li>
          ))}
        </ul>
