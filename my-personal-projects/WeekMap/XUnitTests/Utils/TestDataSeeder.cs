...
namespace XUnitTests.Utils
{
    public static class TestDataSeeder
    {
        ...
        //public static async Task<(long realisedMapId, long activityId)> SeedRealisedWeekMapActivityAsync(HttpClient client)
        //{
        //    // Use similar logic if RealisedWeekMapActivity depends on different data

        //    // This is just an example stub:
        //    var realisedMapDTO = new RealisedWeekMapDTO
        //    {
        //        PlannedWeekMapID = 1,
        //        DateCreated = DateTime.UtcNow
        //    };

        //    var response = await client.PostAsJsonAsync("api/RealisedWeekMap", realisedMapDTO);
        //    response.EnsureSuccessStatusCode();

        //    var realisedList = await client.GetFromJsonAsync<List<RealisedWeekMap>>();
        //    var realisedMapId = realisedList!.First().RealisedWeekMapID;

        //    // Reuse activity from above (or create another one)
        ...
        //    var activityList = await client.GetFromJsonAsync<List<Activity>>();
        //    var activityID = activityList!.First().ActivityID;

        //    return (realisedMapId, activityID);
        //}
    }
}