...
import { ActivityIndicator, Dimensions, FlatList, PanResponder, Pressable, Text, View } from 'react-native';
import { useNavigation } from '@react-navigation/native';
import { ref, onValue, query, orderByChild, equalTo, startAt, endAt, get } from 'firebase/database';
  ...
  const unsub = onValue(
    ...
    (snap) => {
      try {
        ...
        // ---- DEBUG: how many raw candidates
        let rawCount = 0;
        snap.forEach(() => { rawCount++; });
        console.log('[HS] raw candidates from query:', rawCount);
        ...
        snap.forEach((child) => {
          ...
          const debug = { uid: child.key, title: raw.jobTitle, reasons: [] };
          ...
          if (fTitle && !titlePass) debug.reasons.push('title');
          if (fCategory && !categoryPass) debug.reasons.push('category');
          if (fLanguage && !langPass) debug.reasons.push('language');
          if (fQualifications && !qualPass) debug.reasons.push('qualifications');
          if (fTags && !tagsPass) debug.reasons.push('tags');
          if (fLocationName && !locNamePass) debug.reasons.push('locationName');
          if (hasMin && !minPass) debug.reasons.push('minStart');
          if (hasMax && !maxPass) debug.reasons.push('maxEnd');
          if (fScheduleSet.size > 0 && !schedPass) debug.reasons.push('scheduleType');
          if (fWorkSet.size > 0 && !workPass) debug.reasons.push('workModel');

          if (hasDist) {
            if (!jobCoords) debug.reasons.push('noCoords');
            else if (!(typeof distanceKm === 'number' && Number.isFinite(distanceKm))) debug.reasons.push('noDistance');
            else if (distanceKm > maxKm) debug.reasons.push(`tooFar(${distanceKm?.toFixed?.(1)}km)`);
          }

          // recompute distance for display if it passed distance gate
          if (okDist && otherJobLoc && myLoc) {
            distanceKm = getHaversineDistanceKmFrom2LatLngPoints(myLoc, otherJobLoc);
          }
          if (otherJobLoc) jobCoords = otherJobLoc;

          // only include criteria that are set
          const flags = [
